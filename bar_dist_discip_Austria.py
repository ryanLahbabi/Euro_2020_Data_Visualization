...
def prep_distribution_data(df):
    ...
    df_distribution = df[df['PlayerSurname'].isin(['Baumgartner', 'Lainer','Gregoritsch', 'Sabitzer','Arnautovic','Kalajdzic','Trimmel', 'Alaba', 'Schlager', 'Dragovic'])]
...
def prep_disciplanary_data(df):
    ...
    df_disciplanary = df[df['PlayerSurname'].isin(['Baumgartner', 'Lainer','Gregoritsch', 'Sabitzer','Arnautovic','Kalajdzic','Trimmel', 'Alaba', 'Schlager', 'Dragovic'])]
...
def create_disciplanary_plot(df):
    ...
    fig = px.bar(data_frame=df, x="PlayerSurname", y="value", color="Disciplanary",
                title="Austria Disciplinary", 
                hover_data=['percentages',"Disciplanary"],
                text_auto=True)
    

    fig.update_traces(hovertemplate="<b>Player:</b> %{x}<br><b>%{customdata[1]} Completed: </b>%{y} (%{customdata[0]}%)<extra></extra>") 
    fig.update_layout(yaxis={'title': "Disciplinary"},
                      height=660,
