...
def prep_distribution_data(df):
    ...
    df_distribution = df[df['PlayerSurname'].isin(['Kane', 'Sterling', 'Stones','Rice', 'Henderson', 'Grealish', 'Mount', 'Trippier', 'Phillips', 'Walker'])]
...
def prep_disciplanary_data(df):
    ...
    df_disciplanary = df[df['PlayerSurname'].isin(['Kane', 'Sterling', 'Stones','Rice', 'Henderson', 'Grealish', 'Mount', 'Trippier', 'Phillips', 'Walker'])]
...
def create_disciplanary_plot(df):
    ...
    fig.update_traces(hovertemplate="<b>Player:</b> %{x}<br><b>%{customdata[1]} Completed: </b>%{y} (%{customdata[0]}%)<extra></extra>") 
    fig.update_layout(yaxis={'title': "England Disciplinary"},
                      height=660,
